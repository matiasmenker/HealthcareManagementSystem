...
public class Main {
	public static void main(String[] args) {
		SwingUtilities.invokeLater(() -> {
			...
			try {
				...
				ReferralManager referralManager = ReferralManager.getInstance(referralRepository, patientRepository,
						clinicianRepository, facilityRepository, referralProcessingOutputWriter);

				ReferralController referralController = new ReferralController(referralManager, referralRepository,
						patientRepository, clinicianRepository, facilityRepository);
				...
				logStartupInformation(mainFrame, dataDirectory, outputDirectory, patientRepository.findAll().size(),
						clinicianRepository.findAll().size(), facilityRepository.findAll().size(),
						appointmentRepository.findAll().size(), prescriptionRepository.findAll().size(),
						referralRepository.findAll().size(), staffRepository.findAll().size());
	...
	private static void logStartupInformation(MainFrame mainFrame, Path dataDirectory, Path outputDirectory,
			int patientsCount, int cliniciansCount, int facilitiesCount, int appointmentsCount, int prescriptionsCount,
			int referralsCount, int staffCount) {
